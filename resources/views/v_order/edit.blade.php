            ...
            <div class="form-group mb-4">
                ...
                <select class="form-control" id="exampleFormControlSelect1" name="idProduct">
                    @foreach ($products as $product)
                        <option value="{{$product->id}}" {{ $order->idProduct == $product->id ? 'selected' : '' }}>{{$product->name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label">Jumlah Pesanan</label>
                <input type="number" class="form-control" id="exampleInputPassword1" name="amount" placeholder="Masukan Jumlah Pesanan" value="{{$order->amount}}">
            </div>
